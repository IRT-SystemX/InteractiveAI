...
def test_get_all_PowerGrid_contexts(client, create_usecases, create_contexts, PowerGrid_auth_mocker):
...
def test_add_PowerGrid_context(client, create_usecases, PowerGrid_auth_mocker):
    context_data = {
...
def test_get_with_use_case(
    ...
    response = client.get("/api/v1/contexts?use_case=PowerGrid", headers=headers)
    assert response.status_code == 200
...
def test_get_with_use_case_and_date(
    ...
    headers = {"Authorization": f"Bearer {PUBLISHER_TEST_BEARER_TOKEN}"}
    response = client.get(
        "/api/v1/contexts?use_case=PowerGrid&date=2022-11-07T16:06:00.741655",
        headers=headers,
    )
