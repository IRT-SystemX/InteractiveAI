...
class Communicate:
    ...
    def load_config(self):
        """
        Loads the configuration from the 'config/API_RTE_CAB.toml' file.
        """
        try:
            self.outputs_config = toml.load("config/API_RTE_CAB.toml")
            if self.outputs_config['Outputs']['activate'] == 'yes':
                ...
                logging.info(
                    ...
                    "Check the file 'config/API_RTE_CAB.toml' to setup the simulator "
                    "differently."
                )
        except FileNotFoundError:
            logging.error("Le fichier 'config/API_RTE_CAB.toml' n'a pas été trouvé.")
        except toml.TomlDecodeError:
            logging.error("Erreur de décodage du fichier 'config/API_RTE_CAB.toml'.")
    ...
    def edit_parameters(self, parameter_name, new_value):
        ...
        with open("config/API_RTE_CAB.toml", "w", encoding="utf-8") as file:
    ...
    def add_cab_server_url(self, url):
        ...
        try:
            ...
            with open("config/API_RTE_CAB.toml", "w") as configfile:
    ...
    def delete_cab_server_url(self, url):
        ...
        """
        try:
            config_file = "config/API_RTE_CAB.toml"
